...
class GameApp:
    ...
    def finish_move(self):
        ...
        is_capture = (
            move.captured_piece and move.captured_piece.piece_type != PieceType.EMPTY
        )
        ...
        if self.game_over:
            ...
            self.board.check_status[PieceColor.WHITE]
            or self.board.check_status[PieceColor.BLACK]
        ):
    ...
    def draw_board(self):
        ...
        for i in range(4):
            ...
            self.screen.blit(
                lbl,
                (
                    start_x + i * SQUARE_SIZE + SQUARE_SIZE - 15,
        ...
        for row in range(4):
            for col in range(4):
                ...
                if self.last_move:
                    if (row, col) == self.last_move.from_pos or (
                        row,
                        col,
                    ) == self.last_move.to_pos:
                ...
                if self.selected_pos:
                    for move in self.valid_moves_for_selected:
                        if move.to_pos == (row, col):
                            is_capture = (
                                move.captured_piece
                                and move.captured_piece.piece_type != PieceType.EMPTY
                            )
    ...
    def draw_panel(self):
        ...
        pygame.draw.rect(
            self.screen, COLOR_PANEL, (panel_x, 0, PANEL_WIDTH, WINDOW_HEIGHT)
        )
